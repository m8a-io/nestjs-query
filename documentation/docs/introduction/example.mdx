...
Notice the use of `@FilterableField` this will let `@m8a/nestjs-query-graphql` know to allow filtering on the
...
From the previously created DTO, `@m8a/nestjs-query-graphql` will automatically create a `CreateTodoItem` graphql type:
