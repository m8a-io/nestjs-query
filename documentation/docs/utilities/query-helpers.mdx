...
The `@m8a/nestjs-query-core` package provides a number of helper functions to transform or apply queries to a list of items.
...
```ts
import { applyFilter } from `@m8a/nestjs-query-core`;
...
```ts
import { applySort, SortDirection, SortNulls } from `@m8a/nestjs-query-core`;
...
```ts
import { applyPaging } from `@m8a/nestjs-query-core`;
const dtos: TestDTO[] = [
  {first: 'Bob', last: 'Yukon'}
  {first: 'Alice', last: 'Yukon'}
  {first: 'Sally', last: 'Yukon'}
...
```ts
import { applyQuery, SortDirection } from `@m8a/nestjs-query-core`;
...
```ts
import { transformFilter, QueryFieldMap, Filter } from `@m8a/nestjs-query-core`;
...
```ts
import { transformSort, QueryFieldMap, SortField, SortDirection } from `@m8a/nestjs-query-core`;
...
```ts
import { transformQuery, QueryFieldMap, Query, SortDirection } from `@m8a/nestjs-query-core`;
...
```ts
import { Filter, getFilterComparisons } from `@m8a/nestjs-query-core`;
...
```ts
import { Filter, getFilterOmitting } from `@m8a/nestjs-query-core`;
